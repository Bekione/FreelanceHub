...
export default async function PublicInvoicePage({
  ...
  // Public route — accessible via hard-to-guess CUID (no auth required)
  const invoice = await prisma.invoice.findUnique({
    where: { id },
    include: {
      client: true,
    ...
    <div className="min-h-screen bg-white text-black p-4 sm:p-8">
      ...
      <div className="w-full max-w-[800px] bg-white text-black p-10 sm:p-16 shadow-none mx-auto relative">
        <div className="flex justify-between items-start mb-12">
          <div>
            {profile?.brandLogoUrl ? (
              <img src={profile.brandLogoUrl} alt="Brand Logo" className="h-16 w-auto object-contain mb-4" />
            ) : (
              <h1 className="text-3xl font-bold font-heading text-neutral-900">{invoice.user.name}</h1>
            )}
          ...
          <div className="text-right">
            ...
            <div className="text-sm">
              <p className="font-semibold text-neutral-800">
                Invoice #: <span className="font-mono text-neutral-600">{invoice.invoiceNumber}</span>
              </p>
              <p className="text-neutral-500 mt-1">Date: {new Date(invoice.issueDate).toLocaleDateString()}</p>
              <p className="text-neutral-500">
                Due: <span className="font-medium text-neutral-800">{new Date(invoice.dueDate).toLocaleDateString()}</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mb-12 border-t border-neutral-200 pt-8">
          <h3 className="text-xs font-bold uppercase tracking-widest text-neutral-400 mb-3">Bill To</h3>
          <div className="text-sm text-neutral-800">
            <p className="font-semibold text-base">{invoice.client?.name || "N/A"}</p>
            {invoice.client?.company && <p>{invoice.client.company}</p>}
            {invoice.client?.email && <p className="text-neutral-500 mt-1">{invoice.client.email}</p>}
          </div>
        </div>
        ...
        <table className="w-full text-left mb-12 border-collapse">
          ...
          <tbody className="text-sm text-neutral-700 divide-y divide-neutral-100">
            <tr>
              <td className="py-4 px-2">
                <p className="font-medium text-neutral-900">
                  {invoice.project ? `Services rendered for: ${invoice.project.title}` : "Professional Services"}
                </p>
                {invoice.notes && (
                  <p className="text-neutral-500 text-xs mt-1 max-w-sm">{invoice.notes}</p>
                )}
              </td>
            ...
            {Boolean(invoice.bonus) && invoice.bonus! > 0 && (
              <tr>
                <td className="py-4 px-2">
                  <p className="font-medium text-neutral-900">Bonus / Additional Fees</p>
                </td>
                <td className="py-4 px-2 text-right font-medium text-neutral-900">
